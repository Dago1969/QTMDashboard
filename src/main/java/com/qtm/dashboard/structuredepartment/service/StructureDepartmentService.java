 ...
 * i reparti delle strutture locali.
 */
@Slf4j
@Service
public class StructureDepartmentService {
	...
	StructureDepartmentService(ASLRepository aslRepository, HospitalRepository hospitalRepository,
			StructureDepartmentRepository structureDepartmentRepository, RestClient restClient,
			RestClient apiRootRestClient, String ticketBaseUrl) {
	...
	public StructureDepartmentSourceDto findById(Long id) {
		log.info("[StructureDepartmentService] ricerca reparto struttura id={}", id);
		if (id == null)
			return null;

		// Se salvato nel DB locale, crea il DTO dall'Entity
		Optional<StructureDepartmentEntity> localEntity = structureDepartmentRepository.findById(id);
	...
	public StructureDepartmentFilterOptionsDto getFilterOptions() {
		...
		return StructureDepartmentFilterOptionsDto.builder().regions(buildRegionOptions(regionsByCode))
				.asls(buildAslOptions(asls, regionsByCode))
				.hospitals(buildHospitalOptions(hospitalsByStructureCode, aslsById)).build();
	}

	@Transactional(readOnly = true)
	public List<StructureDepartmentOverviewDto> findAllWithImportStatus(String regionCode, String aslCode,
			String structureCode) {
		log.info("[StructureDepartmentService] overview region={} asl={} structure={}", regionCode, aslCode,
				structureCode);

		Map<String, RegionDto> regionsByCode = loadRegionsByCode();
		List<ASLEntity> asls = aslRepository.findAll();
		Map<String, ASLEntity> aslsByRegionAndCode = asls.stream().filter(
				asl -> StringUtils.hasText(asl.getCodiceRegione()) && StringUtils.hasText(asl.getCodiceAzienda()))
				.collect(Collectors.toMap(asl -> buildAslKey(asl.getCodiceRegione(), asl.getCodiceAzienda()),
		...
		List<String> associatedStructureCodes = resolveAssociatedStructureCodes(regionCode, aslCode, structureCode,
				hospitalsByStructureCode, aslsById);
		...
		List<StructureDepartmentSourceDto> ticketDepartments = associatedStructureCodes.stream()
				.peek(code -> log.info(
						"[StructureDepartmentService] fetching departments from TICKET for structureCode={}", code))
				.flatMap(code -> fetchStructureDepartmentsByStructureCode(code).stream()).toList();
		log.info("[StructureDepartmentService] extracted {} departments from TICKET keys={}", ticketDepartments.size(),
				ticketDepartments.stream()
						.map(row -> buildAssociationKey(row.getCodiceStruttura(), row.getCodiceDisciplina())).toList());

		List<StructureDepartmentOverviewDto> overview = ticketDepartments.stream()
				.map(sourceDepartment -> toOverview(sourceDepartment, regionsByCode, aslsByRegionAndCode, aslsById,
						hospitalsByStructureCode, localAssociationsByKey))
				.toList();
		log.info("[StructureDepartmentService] enriched overview rows={} importedKeys={}", overview.size(),
				overview.stream().filter(row -> Boolean.TRUE.equals(row.getImported()))
						.map(row -> buildAssociationKey(row.getCodiceStruttura(), row.getCodiceDisciplina())).toList());
		return overview;
	}
	...
	public void importAssociation(StructureDepartmentImportRequest request) {
		String codiceStruttura = normalizeRequiredCode(request != null ? request.getCodiceStruttura() : null,
				"Codice struttura mancante");
		String codiceDisciplina = normalizeRequiredCode(request != null ? request.getCodiceDisciplina() : null,
				"Codice disciplina mancante");

		log.info("[StructureDepartmentService] associazione reparto struttura={} disciplina={}", codiceStruttura,
				codiceDisciplina);

		StructureDepartmentSourceDto sourceDepartment = fetchStructureDepartmentsByStructureCode(codiceStruttura)
				.stream().filter(row -> Objects.equals(normalizeCode(row.getCodiceDisciplina()), codiceDisciplina))
				.findFirst()
				.orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND,
						String.format("Reparto %s non trovato in QTMTicket per la struttura %s.", codiceDisciplina,
								codiceStruttura)));
	...
	public void deleteAssociation(String codiceStruttura, String codiceDisciplina) {
		...
		if (!structureDepartmentRepository.existsByCodiceStrutturaAndCodiceDisciplina(normalizedStructureCode,
				normalizedDepartmentCode)) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,
					String.format("Associazione non trovata per struttura %s e disciplina %s.", normalizedStructureCode,
							normalizedDepartmentCode));
		}

		structureDepartmentRepository.deleteByCodiceStrutturaAndCodiceDisciplina(normalizedStructureCode,
				normalizedDepartmentCode);
	}

	private StructureDepartmentOverviewDto toOverview(StructureDepartmentSourceDto sourceDepartment,
			Map<String, RegionDto> regionsByCode, Map<String, ASLEntity> aslsByRegionAndCode,
			Map<Long, ASLEntity> aslsById, Map<String, HospitalEntity> hospitalsByStructureCode,
		...
		String associationKey = buildAssociationKey(sourceDepartment.getCodiceStruttura(),
				sourceDepartment.getCodiceDisciplina());

		return StructureDepartmentOverviewDto.builder().id(sourceDepartment.getId()).codiceRegione(effectiveRegionCode)
				.regione(region != null ? region.getName() : null).codiceAsl(resolvedAslCode)
	...
	private Map<String, RegionDto> loadRegionsByCode() {
		return fetchAllRegionsFromTicket().stream().filter(region -> StringUtils.hasText(region.getCode()))
				.collect(Collectors.toMap(region -> normalizeCode(region.getCode()), Function.identity(),
						(first, second) -> first));
	}
	...
	private Map<String, HospitalEntity> loadHospitalsByStructureCode() {
		return hospitalRepository.findAll().stream()
				...
				.collect(Collectors.toMap(hospital -> normalizeCode(hospital.getCodiceStruttura()), Function.identity(),
						(first, second) -> first));
	}

	private List<String> resolveAssociatedStructureCodes(String regionCode, String aslCode, String structureCode,
			Map<String, HospitalEntity> hospitalsByStructureCode, Map<Long, ASLEntity> aslsById) {
		...
		List<String> resolvedStructureCodes = hospitalsByStructureCode.values().stream()
				.filter(hospital -> StringUtils.hasText(hospital.getCodiceStruttura()))
				.filter(hospital -> normalizedStructureCode == null
						|| Objects.equals(normalizeCode(hospital.getCodiceStruttura()), normalizedStructureCode))
				...
				.map(HospitalEntity::getCodiceStruttura).map(this::normalizeCode).filter(Objects::nonNull).distinct()
				.sorted().toList();
		log.info("[StructureDepartmentService] resolved hospital structure codes from QTMDB hospital={}",
				resolvedStructureCodes);
	...
	private boolean matchesRegion(String regionCode, HospitalEntity hospital, Map<Long, ASLEntity> aslsById) {
		return regionCode == null
				|| Objects.equals(normalizeCode(resolveHospitalRegionCode(hospital, aslsById)), regionCode);
	}
	...
	private Map<Long, ASLEntity> loadAslsById(List<ASLEntity> asls) {
		return asls.stream().filter(asl -> asl.getId() != null)
	...
	private List<StructureDepartmentFilterOptionDto> buildRegionOptions(Map<String, RegionDto> regionsByCode) {
		return regionsByCode.values().stream()
				...
				.map(region -> StructureDepartmentFilterOptionDto.builder().code(region.getCode())
						.label(StringUtils.hasText(region.getName()) ? region.getName() + " (" + region.getCode() + ")"
								: region.getCode())
						.regionCode(region.getCode()).build())
				.toList();
	}

	private List<StructureDepartmentFilterOptionDto> buildAslOptions(List<ASLEntity> asls,
			Map<String, RegionDto> regionsByCode) {
		return asls.stream().filter(asl -> StringUtils.hasText(asl.getCodiceAzienda())).sorted(Comparator
				.comparing(ASLEntity::getDenominazioneAzienda, Comparator.nullsLast(String::compareToIgnoreCase)))
				.map(asl -> {
					...
					return StructureDepartmentFilterOptionDto.builder().code(asl.getCodiceAzienda()).label(label)
							.regionCode(asl.getCodiceRegione()).aslCode(asl.getCodiceAzienda()).build();
				}).toList();
	}
	...
	private List<StructureDepartmentFilterOptionDto> buildHospitalOptions(
		...
		return hospitalsByStructureCode.values().stream()
				.filter(hospital -> StringUtils.hasText(hospital.getCodiceStruttura())).sorted(Comparator
						.comparing(HospitalEntity::getStruttura, Comparator.nullsLast(String::compareToIgnoreCase)))
				.map(hospital -> {
					...
					return StructureDepartmentFilterOptionDto.builder().code(hospital.getCodiceStruttura()).label(label)
							.regionCode(resolvedRegionCode).aslCode(resolvedAslCode).build();
				}).toList();
	}
	...
	private List<RegionDto> fetchAllRegionsFromTicket() {
		...
		try {
			RegionDto[] sourceRegions = apiRootRestClient.get().uri("regions").retrieve().body(RegionDto[].class);
	...
	private List<StructureDepartmentSourceDto> fetchStructureDepartmentsByStructureCode(String codiceStruttura) {
		...
		try {
			StructureDepartmentSourceDto[] sourceDepartments = restClient.get()
					.uri("structure-departments?codiceStruttura={codiceStruttura}", codiceStruttura).retrieve()
					.body(StructureDepartmentSourceDto[].class);
			List<StructureDepartmentSourceDto> result = sourceDepartments == null ? new ArrayList<>()
					: Arrays.asList(sourceDepartments);
			log.info("[StructureDepartmentService] ticket rows for structure={} count={} keys={}", codiceStruttura,
					result.size(),
					result.stream().map(row -> buildAssociationKey(row.getCodiceStruttura(), row.getCodiceDisciplina()))
							.toList());
			return result;
		} catch (Exception exception) {
			log.error("[StructureDepartmentService] errore fetchStructureDepartmentsByStructureCode struttura={}",
					codiceStruttura, exception);
			throw mapExternalException(exception, targetUrl);
		}
	}
	...
	private ResponseStatusException mapExternalException(Exception exception, String targetUrl) {
		...
		if (exception instanceof RestClientResponseException restClientResponseException) {
			String detail = String.format("QTMTicket ha risposto con stato %s durante la chiamata %s.",
					restClientResponseException.getStatusCode().value(), targetUrl);
	...
	/**
	 * Identifica univocamente un'ASL perché il codice azienda può ripetersi tra
	 * regioni diverse.
	 */
